...
Written defensively: some live environments were bootstrapped via
Base.metadata.create_all (app/database.py imports billing models now) before
this Alembic revision ran, so the billing_* tables may already exist there.
Each table — and the indexes that belong to it — is created only when the
table is absent, so a fresh database gets the full schema while an already
seeded database degrades this revision to a no-op instead of aborting with
duplicate-table/index errors. No existing table is altered or dropped.
"""
...
def upgrade() -> None:
    ...
    if 'billing_plan_versions' not in existing_tables:
        op.create_table(
    ...
    if 'billing_entitlement_flags' not in existing_tables:
        op.create_table(
    ...
    if 'billing_entitlement_overrides' not in existing_tables:
        op.create_table(
    ...
    if 'billing_price_catalog_items' not in existing_tables:
        op.create_table(
    ...
    if 'billing_subscription_items' not in existing_tables:
        op.create_table(
    ...
    if 'billing_worker_month_records' not in existing_tables:
        op.create_table(
    ...
    if 'billing_invoices' not in existing_tables:
        op.create_table(
            'billing_invoices',
            sa.Column('id', sa.Integer(), primary_key=True),
            sa.Column('organization_id', sa.Integer(), nullable=False),
    ...
    if 'billing_invoice_lines' not in existing_tables:
        op.create_table(
    ...
    if 'billing_credit_notes' not in existing_tables:
        op.create_table(
    ...
    if 'billing_dunning_state' not in existing_tables:
        op.create_table(
    ...
    if 'billing_commercial_audit_events' not in existing_tables:
        op.create_table(
