...
Three independent statutory obligations (never folded into one number):
  ...
  2. Health Surcharge — a flat WEEKLY amount (TT$8.25 if monthly
     ...
     Weekly-paid employee, else TT$4.80), not a percentage of pay. Age/
     pension-only exemptions (TT-008) are
     deferred. Phase 1 assumes 4 contribution weeks per Monthly pay
     period — the exact same simplifying assumption the spec's own §12
     worked fixtures use ("Health Surcharge and NIS examples assume four
     liable/contribution weeks in the illustrated month"); a real
...
def calculate(ctx: PayrollContext) -> dict:
    ...
    professional_tax = _round2(weekly_rate * weeks_in_period)  # Health Surcharge — see module docstring
