...
SAFETY_GUARDRAIL = (
    ...
    "offer one focused next step or human handoff."
)
...
def build_evidence_envelope(
    ...
    for idx, item in enumerate(evidence, start=1):
        lines.append(
            f"[EVIDENCE {idx}] type={item.get('source_type')} title={item.get('title')} "
            f"effective={item.get('effective_at')} authority={item.get('authority')}"
        )
    for idx, item in enumerate(knowledge, start=1):
        lines.append(
            f"[KNOWLEDGE {idx}] title={item.get('title')} summary={item.get('summary')} body={item.get('body')}"
        )
