...
// Super Admin > Statutory Rates
// --------------------------------
// A simpler, numeric-values-only view over the exact same canonical
// ContributionRate/TaxSlab data Compliance manages — creating a new Tax
// pack, changing its status, or version history all happen on Compliance
// instead; this page is just a fast way to see and quick-edit the actual
// numbers currently in force for a jurisdiction.

const inputClass = "w-full rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-focus-ring/30";
...
export default function StatutoryRatesPage() {
  ...
  useEffect(() => {
    getComplianceJurisdictions().then((rows) => {
      setJurisdictions(rows);
      if (rows[0]) setCountry(rows[0].code);
    });
  }, []);
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <select className={inputClass + " w-auto min-w-[160px]"} value={country} onChange={(e) => { setCountry(e.target.value); setState(""); }}>
