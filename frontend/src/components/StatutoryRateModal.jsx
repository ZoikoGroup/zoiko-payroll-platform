...
function emptyForm(defaultCountry, defaultState) {
  return {
    jurisdiction_country: defaultCountry || "IN",
    jurisdiction_state: defaultState || "",
    component_key: "",
    label: "",
    employee_share: "",
    employer_share: "",
...
function toForm(rate, defaultCountry, defaultState) {
  if (!rate) return emptyForm(defaultCountry, defaultState);
  return {
    jurisdiction_country: rate.jurisdiction_country,
    jurisdiction_state: rate.jurisdiction_state || "",
      ...
      <div className="bg-white rounded-xl shadow-lg w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4">
          <label className="block">
            ...
            <input
              className={`${INPUT} ${lockJurisdiction ? "opacity-60" : ""}`}
              value={form.jurisdiction_country}
              onChange={set("jurisdiction_country")}
              disabled={lockJurisdiction}
            />
          </label>
          <label className="block">
            <span className="text-xs font-medium text-slate-600">State / Province (optional)</span>
            <input
              className={`${INPUT} ${lockJurisdiction ? "opacity-60" : ""}`}
              value={form.jurisdiction_state}
              onChange={set("jurisdiction_state")}
              disabled={lockJurisdiction}
              placeholder="Blank = country-level"
            />
          </label>
